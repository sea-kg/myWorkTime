...
if (mwtBase::issetParam('start_time') && mwtBase::issetParam('stop_time'))
{
	...
	try {
		...
		$stmt1 = $conn->prepare('SELECT TIME( SUM( TIMEDIFF( stop_time, start_time ) ) ) as sum FROM realtime WHERE start_time >= ? AND stop_time <= ? AND userid = ?');
		...
		$stmt = $conn->prepare('select *, TIMEDIFF( stop_time, start_time ) as d  from realtime where start_time >= ? and stop_time <= ? and userid = ? ORDER BY start_time');
